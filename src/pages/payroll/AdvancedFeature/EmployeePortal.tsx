...
import { formatCurrency } from "../const";
import { employeePortalService } from "../../../services/modules/payrollServices/employeePortal";
...
export default function EmployeePortal() {
  ...
  const loadData = async () => {
    ...
    showSpinner();
    try {
      const [summaryRes, featuresRes, employeesRes]: any = await Promise.all([
        employeePortalService.getPortalSummary(),
        employeePortalService.getPortalFeatures(),
        employeePortalService.getPortalEmployees({ size: 50 }),
      ]);
  ...
  return (
    <Box sx={{ p: 3, bgcolor: "background.default", minHeight: "100vh" }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        {[
          { label: "Total Employees", value: summary?.total || employees.length, color: "#3b82f6", icon: <PersonIcon /> },
                  ...
                  {filteredEmployees.length === 0 ? (
                    ...
                    filteredEmployees.map((emp) => (
                      <TableRow key={emp.employeeId} hover>
                        ...
                        <TableCell>
                          <Chip label={emp.status || "Active"} size="small" color={emp.status === "Active" ? "success" : "warning"} />
                        </TableCell>
                        <TableCell align="center">
                          <Stack direction="row">
                            <IconButton size="small" onClick={() => { setDialogType("payslip"); setOpenDialog(true); }}>
                              <ReceiptIcon fontSize="small" />
                            </IconButton>
                            <IconButton size="small" onClick={() => { setDialogType("bank"); setOpenDialog(true); }}>
                              <BankIcon fontSize="small" />
                            </IconButton>
                          </Stack>
                        </TableCell>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          {dialogType === "bank" && (
            <Button onClick={handleUpdateBank} variant="contained" sx={{ textTransform: "none" }}>
              Save
            </Button>
          )}
        </DialogActions>
