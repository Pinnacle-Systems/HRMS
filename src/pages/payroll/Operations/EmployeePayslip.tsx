...
//       if (id) {
//         try {
//           const res = await payslipsService.viewPayslip(id);
//           setPayslipData(res.data);
//         } catch (error) {
//           showSnackbar("Failed to load payslip", "error");
//         }
//       }
//     };
//     loadPayslip();
...
export default function EmployeePayslip() {
  ...
  const loadPayslip = async () => {
    ...
    showSpinner();
    try {
      const [year, month] = decodedPeriod.split(" ").map((p, i) => {
        if (i === 0) {
          const monthMap: Record<string, number> = {
            Jan: 1, Feb: 2, Mar: 3, Apr: 4, May: 5, Jun: 6,
            Jul: 7, Aug: 8, Sep: 9, Oct: 10, Nov: 11, Dec: 12
          };
          return monthMap[p.substring(0, 3)];
        }
        return parseInt(p);
      });
      // const res = await payslipsService.getPayslipByEmployeeAndPeriod(employeeId, year, month);
      // setPayslip(res.data);
    } catch (error: any) {
  ...
  const handleDownload = async () => {
    ...
    //     return parseInt(p);
    //   });
    //   const res = await payslipsService.downloadPayslip(employeeId, year, month);
    ...
    <Box sx={{ p: 3, bgcolor: "background.default", minHeight: "100vh" }}>
      ...
      >
        ...
        <Box sx={{ px: 4, py: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <Grid container spacing={2}>
            {[
              ["Employee ID", employeeData.employeeCode || employeeId],
