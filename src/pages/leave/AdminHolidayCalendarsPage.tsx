...
import { CloudUploadOutlined, Delete, Edit } from "@mui/icons-material";
...
export default function AdminHolidayCalendarsPage() {
  ...
  const [importFile, setImportFile] = useState<File | string>("");
  ...
  const submitImport = async () => {
    if (!(importFile instanceof File)) {
      showSnackbar("Select a file to import", "error");
      return;
    }
    if (!importCalendarId) {
    ...
    showSpinner();
    try {
      const response = await leaveService.importHolidays(
        importFile,
        importCalendarId,
      );
      if (response.success) {
        showSnackbar(response.message || "Holidays imported", "success");
        setImportOpen(false);
        setImportFile("");
        setImportCalendarId("");
        await load();
  ...
  return (
    ...
    >
            ...
            {!loading &&
              calendars.map((calendar, i) => (
                <TableRow key={calendar.id} hover sx={getRowColor(i)}>
                  ...
                  <TableCell>{/* {calendar.holidayCount || 0} */}0</TableCell>
                  <TableCell>
                    <Chip
      ...
      >
        <div className="flex items-center justify-between p-2 border-b border-gray-300">
          <div className="text-gray-800 ml-4">Import Holidays</div>
        ...
        <DialogContent className="!p-6">
          <div className="grid gap-4">
            <TextField
              ...
              onChange={(event) => setImportCalendarId(event.target.value)}
              required
            >
            ...
            {/* <FileUpload
              label="Holiday File (CSV)"
              value={importFile}
              onChange={setImportFile}
              accept=".csv,.xlsx"
              maxSize={5}
              compact
              description="Upload a CSV/XLSX file with holiday name, date, type, and location columns."
            /> */}
