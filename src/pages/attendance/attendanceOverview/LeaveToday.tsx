...
export function LeaveToday() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      setLeaveTypes(Array.isArray(res1.data?.content || res1.data) ? (res1.data?.content || res1.data) : []);
    } catch {
      // ignore
    }
  };
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-gray-800">Employees on Leave Today</span>
          <Chip label={leaves.length} size="small" color="primary" />
        </div>
        <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="text-center py-4 text-gray-500 text-[12px]">No employees on leave today</div>
      ) : (
        <TableContainer className="max-h-[300px]">
          <Table size="small" stickyHeader>
            <TableHead>
              <TableRow>
                {["S No", "Employee", "Code", "Department", "Leave Type", "Period"].map((h) => (
            ...
            <TableBody>
              {leaves.map((l, i) => (
                <TableRow key={l.employeeId} hover sx={getRowColor(i)}>
                  <TableCell>{i + 1}</TableCell>
                  <TableCell className="whitespace-nowrap">{l.employeeName}</TableCell>
